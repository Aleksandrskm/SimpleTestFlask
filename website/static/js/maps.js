...
getRowsTable('ZN',0,99999).then(zone=>{
    ...
    postJSON(data).then(tableInfo=>{
        for( let i=0;i<tableInfo.columns.length;i++){
            ...
            document.querySelector('.district-Table thead').append(th);
        }
        ...
        const keysRusName=(Object.keys(rusName));
            for(const key in zone[i])
            {
                ...
                { 
                    ...
                    elZN.innerHTML+=`<span class="${rusName[name]}">${rusName[name]}:${(zone[i][key])}</span>`
                }
       ...
       document.querySelectorAll('.zn-element').forEach((zn) => {
        zn.addEventListener('click',(e)=>{
           const latLN=zn.children[5].innerHTML.substring(zn.children[4].innerHTML.indexOf(':')+17);
           const lonLN=zn.children[6].innerHTML.substring(zn.children[5].innerHTML.indexOf(':')+2);
           const latPV=zn.children[7].innerHTML.substring(zn.children[6].innerHTML.indexOf(':')+2);
           const lonPV=zn.children[8].innerHTML.substring(zn.children[7].innerHTML.indexOf(':')+2);
