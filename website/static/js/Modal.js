...
export class Modal{
    ...
    */
    ...
    createModal(callback){
        ...
        // НАЧАЛО ЦИКЛА - пропускаем первый столбец (ID), так как он автоинкрементный
        for (let i = 1; i < this.colInputs; i++) {
            const modalInput=document.createElement('input');

            // ПРОВЕРКА: для insert операции tableRow может быть null
            let value = '';
            if (this.tableRow && this.tableRow.children && this.tableRow.children[i]) {
                value = String(this.tableRow.children[i].innerHTML);
            }
        ...
        if (this.typeModal=='insert') {
          ...
          <button class="btn modal__close btn_dark btn_min">Отмена</button></div>`;
            ...
            // ПРОВЕРКА: для delete операции tableRow не должен быть null
            const rowId = this.tableRow && this.tableRow.children ? this.tableRow.children[0].innerHTML : 'неизвестно';
            modalContent.innerHTML+=` <div class="copy__row">Удаляется строка ${rowId}</div>`;
            modalContent.innerHTML+=` 
          <div class='btnsModal'>
